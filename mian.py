...
# Past 24 hours filter (Filters modal — Date posted) — fallback if Latest not found; resourceId wen
PAST_24_HOURS_SELECTORS = [
    ("description", "Past 24 hours"),
    ("text", "Past 24 hours"),
    ("resourceId", "com.zhiliaoapp.musically:id/wen"),
    ("className_text", "android.widget.Button", "Past 24 hours"),
]
...
def click_past_24_hours_button(d, timeout=3.0):
    """Click the 'Past 24 hours' filter button in the Filters modal (Date posted). Returns True if clicked."""
    for item in PAST_24_HOURS_SELECTORS:
        kind = item[0]
        try:
            if kind == "description":
                el = d(description=item[1])
    ...
    for xpath in [
        '//*[@text="Past 24 hours"]',
        '//*[@description="Past 24 hours"]',
        '//*[@resource-id="com.zhiliaoapp.musically:id/wen"]',
        '//android.widget.Button[@text="Past 24 hours"]',
    ]:
        try:
            d.xpath(xpath).click()
            return True
